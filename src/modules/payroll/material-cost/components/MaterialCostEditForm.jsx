...
export const MaterialCostEditForm = ({
  materialId,
  visible,
  onHide,
  onSuccess,
  ...
  const [updateMaterial] = useMutation(UPDATE_MATERIAL_COST);

  const [getMaterial, { loading }] = useLazyQuery(GET_MATERIAL_COST, {
    variables: { id: materialId },
    skip: !materialId || !visible,
    ...
    onCompleted: (data) => {
      if (data?.materialCost) {
        ...
        setFormData({
          ...
          currency: material.currency?.id || null,
          isActive: material.isActive,
          businessId: material.business?.id || null,
          officeId: material.office?.id || null,
          departmentId: material.department?.id || null,
          teamId: material.team?.id || null,
        });
      }
    },
    onError: (err) => {
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Error al cargar el material",
        life: 3000,
      });
    },
  });
